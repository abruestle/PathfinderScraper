...
//Initializing
  ...
  mongoose.connect("mongodb://localhost/pathfinderscraper", {
    useMongoClient: true
  });
// Routes
...
app.get("/scrapecore", function(req, res) {
  ...
  axios.get("http://www.d20pfsrd.com/classes/core-classes/").then(function(response) {
    ...
    $("ul.ogn-childpages li").each(function(i, element) {
      ...
      result.title = $(this)
        .children("a")
        .text();
      //href if it has it replaces first part of link...as not all links will have it, so we are always assuming they don't.
      result.link = $(this)
        .children("a")
